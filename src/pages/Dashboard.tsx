...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      {/* API Key + Python requirement side by side */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-[#111318] border border-[#1e2130] rounded-lg p-5">
          <h2 className="font-bold text-sm text-white mb-3">
            <span className="text-yellow-400 mr-1">⚠️</span> Python 3.11 Required
          </h2>
          <p className="text-xs text-[#7a8099] mb-4">
            PyTorch does not support Python 3.12/3.13 yet.{' '}
            <a href="https://www.python.org/downloads/release/python-3119/" target="_blank" rel="noopener noreferrer" className="text-[#5b8dee] hover:underline">
              Download Python 3.11
            </a>
            {' '}— during install, do <span className="text-white font-medium">not</span> check "Add to PATH".
          </p>
          <div className="space-y-2">
            {[
              { label: 'Install agent on Python 3.11', cmd: 'py -3.11 -m pip install benchgr-agent' },
              { label: 'Remove CPU-only PyTorch (if installed)', cmd: 'py -3.11 -m pip uninstall torch -y' },
              { label: 'Install PyTorch with CUDA', cmd: 'py -3.11 -m pip install torch --index-url https://download.pytorch.org/whl/cu124' },
              { label: 'Verify GPU is detected', cmd: 'py -3.11 -c "import torch; print(torch.cuda.is_available())"' },
            ].map(({ label, cmd }) => (
              <div key={cmd}>
      ...
      <div className="bg-[#111318] border border-[#1e2130] rounded-lg p-5">
        ...
        <div className="space-y-3">
          {[
            { step: '1', label: 'Install the agent (Python 3.11)', cmd: 'py -3.11 -m pip install benchgr-agent' },
            { step: '2', label: 'Save your API key', cmd: `py -3.11 -m benchgr_agent.main config set-key ${user?.api_key ?? 'YOUR_KEY'}` },
            { step: '3', label: 'Run benchmarks & submit', cmd: 'py -3.11 -m benchgr_agent.main run' },
          ].map(({ step, label, cmd }) => (
